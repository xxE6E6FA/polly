...
export const OPTIONAL_REASONING_PATTERNS = [
  ...
  // OpenRouter reasoning models (vary by model, most support effort levels or max_tokens)
  "qwen2.5",
...
function getProviderSpecificPatterns(
  ...
  return patterns.filter(pattern => {
    ...
    switch (provider) {
      case "openai":
        return patternLower.includes("o1-") || 
...
export const PROVIDER_REASONING_CONFIG = {
  openai: {
    getMandatoryPatterns: () => getProviderSpecificPatterns(MANDATORY_REASONING_PATTERNS, "openai"),
    getOptionalPatterns: () => getProviderSpecificPatterns(OPTIONAL_REASONING_PATTERNS, "openai"),
    configFormat: { reasoning: true },
  },
  google: {
    getMandatoryPatterns: () => getProviderSpecificPatterns(MANDATORY_REASONING_PATTERNS, "google"),
    getOptionalPatterns: () => getProviderSpecificPatterns(OPTIONAL_REASONING_PATTERNS, "google"),
    configFormat: { thinkingConfig: { thinkingBudget: 'number' } },
  },
  anthropic: {
    getMandatoryPatterns: () => getProviderSpecificPatterns(MANDATORY_REASONING_PATTERNS, "anthropic"),
    getOptionalPatterns: () => getProviderSpecificPatterns(OPTIONAL_REASONING_PATTERNS, "anthropic"),
    configFormat: { thinking: { type: "enabled", budgetTokens: 'number' } },
  },
  openrouter: {
    getMandatoryPatterns: () => getProviderSpecificPatterns(MANDATORY_REASONING_PATTERNS, "openrouter"),
    getOptionalPatterns: () => getProviderSpecificPatterns(OPTIONAL_REASONING_PATTERNS, "openrouter"),
    configFormat: { 
      reasoning: { 
...
export function hasMandatoryReasoning(
  ...
  if (provider === "openrouter") {
    ...
    if (openrouterConfig.getMandatoryPatterns().some((pattern) =>
    ...
    for (const [providerKey, config] of Object.entries(PROVIDER_REASONING_CONFIG)) {
      if (providerKey !== "openrouter") {
        const hasProviderPattern = config.getMandatoryPatterns().some((pattern) =>
          modelIdLower.includes(pattern.toLowerCase())
        );
  ...
  const providerConfig = PROVIDER_REASONING_CONFIG[provider as keyof typeof PROVIDER_REASONING_CONFIG];
  if (providerConfig?.getMandatoryPatterns) {
    const hasProviderPattern = providerConfig.getMandatoryPatterns().some((pattern) =>
      modelIdLower.includes(pattern.toLowerCase())
    );
    if (hasProviderPattern) {
      return true;
...
export function hasOptionalReasoning(
  ...
  if (provider === "openrouter") {
    ...
    const openrouterConfig = PROVIDER_REASONING_CONFIG.openrouter;
    if (openrouterConfig.getOptionalPatterns().some((pattern) =>
      modelIdLower.includes(pattern.toLowerCase())
    )) {
      return true;
    }
    ...
    for (const [providerKey, config] of Object.entries(PROVIDER_REASONING_CONFIG)) {
      if (providerKey !== "openrouter") {
        const hasProviderPattern = config.getOptionalPatterns().some((pattern) =>
          modelIdLower.includes(pattern.toLowerCase())
        );
        if (hasProviderPattern) {
          return true;
  ...
  if (providerConfig?.getOptionalPatterns) {
    const hasProviderPattern = providerConfig.getOptionalPatterns().some((pattern) =>
      modelIdLower.includes(pattern.toLowerCase())
    );
    if (hasProviderPattern) {
      return true;
...
export function supportsReasoning(
  ...
  return hasMandatoryReasoning(provider, modelId) || hasOptionalReasoning(provider, modelId);
}
 ...
 */
export function getProviderReasoningRequirements(provider: string): any {
  const config = PROVIDER_REASONING_CONFIG[provider as keyof typeof PROVIDER_REASONING_CONFIG];
