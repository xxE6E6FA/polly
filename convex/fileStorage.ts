...
/**
 * Debug query to check what data we have
 */
export const debugUserFiles = query({
  args: {},
  handler: async ctx => {
    const userId = await getAuthUserId(ctx);
    if (!userId) {
    ...
    console.log(
      `[debugUserFiles] User ${userId} has ${conversations.length} conversations`
    );

    // Get a sample of messages with attachments
    ...
    for (const conv of conversations.slice(0, 3)) {
      ...
      sampleMessages.push(...messages.slice(0, 2)); // Take first 2 messages with attachments from each
    }
    ...
    const debugInfo = {
      ...
      sampleMessages: sampleMessages.map(m => ({
        ...
        attachments: m.attachments?.map(a => ({
          name: a.name,
          type: a.type,
          hasStorageId: !!a.storageId,
          storageId: a.storageId,
          hasContent: !!a.content,
          contentLength: a.content?.length || 0,
          hasGeneratedImage: !!a.generatedImage,
          isGenerated: a.generatedImage?.isGenerated,
        })),
      })),
    };

    console.log("[debugUserFiles]", JSON.stringify(debugInfo, null, 2));
    return debugInfo;
  },
});
