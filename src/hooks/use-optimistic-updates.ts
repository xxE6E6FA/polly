...
export function useOptimisticToggle<TItem extends Record<string, unknown>>({
  ...
  return useOptimisticUpdate<TItem[], Record<string, unknown>>({
    convexMutation,
    queryKey,
...
export function useOptimisticItemUpdate<TItem extends Record<string, unknown>>({
  convexMutation,
  queryKey,
  itemId,
  onError,
...
export const useOptimisticMutation = useOptimisticUpdate;
